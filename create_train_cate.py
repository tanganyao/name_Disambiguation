...
import time,re,sys,urllib2,urllib,optparse,json,glob,operator,os,codecs,shutil
...
l_person = []    #同名人物列表
l_index = []    #重名人物索引
company_and_name = []   #以如：王石_万科公司，保存标记
...
def getSegString(strcontent):
    ...
    article = json.loads(strarticle, strict=False)
    postjson = json.loads('{"pos": "1", "utf": "1", "entity": "1", "merge": "1", "rtiment": "1", "rentity": "1"}')
    postjson['article'] = article
    req = urllib2.urlopen('http://21.raisound.com:12550', json.dumps(postjson).encode('utf-8'))
...
def doParse(linestr, index):
    ...
    p = person(l_content)
    company_and_name.append(l_content[1]+'_'+l_content[0])
    #print p
    if len(l_content) < 3:
...
def result_output(file_path):
    ...
    for i,line in enumerate(l_index):
        if company_and_name[line[0]].split("_")[0] not in person_cate and len(company_and_name[line[0]].split("_")[0])>1:
            person_cate.append(company_and_name[line[0]].split("_")[0])
            catename_in_name = "news_data"+"/"+company_and_name[line[0]].split("_")[0]
            creat_cate(catename_in_name)
        else:#已经
            person_cate = []
            person_cate.append(company_and_name[line[0]].split("_")[0])
        for index,single in enumerate(line):
            class_name = catename_in_name +"/"+ company_and_name[single]
            if index == 0:
                creat_cate(class_name)
                first_in_cate_name = class_name
            if index > 0:
                class_name = first_in_cate_name
            with codecs.open(class_name+'/'+company_and_name[single]+".txt", 'w', 'utf-8') as f:
                #f.write(file.readlines()[single])
                with codecs.open(file_path, 'r', 'utf-8')as f_r:
                    f.write(''.join(f_r.readlines()[single:single+1]))
        print(str(i)+"*************************")
...
def readfile(file_path):
    with codecs.open(file_path, 'r', 'utf-8') as f:
        for index, line in enumerate(f.readlines()[:19997]):
            #print(line)
            ...
            #break
	    print(str(index)+"@@@@@@@@@@@@@@@@@@")
    result_output(file_path)
    print("运行完成")
   #print l_person[1][1].person_data

def distin(file_path='output_data/samename_out_sameperson.txt'):
    #file_path = "output_data/samename_out_sameperson.txt"               
    readfile(file_path)
...
if __name__ == "__main__":
    #person_name = '王石' 
    file_path = "output_data/samename_out_sameperson.txt"               
    #readfile(file_path, person_name)            
    readfile(file_path)
'''
##############################################################################################################
def command_line():
